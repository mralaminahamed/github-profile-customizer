...
export const ProfileTab: React.FC<TabProps> = ({ settings, onSettingChange }) => {
  ...
  const handleBulkAction = React.useCallback(
    (action: 'hideAll' | 'showAll', sectionIds: string[]) => {
  ...
  const stats = React.useMemo(() => {
    ...
    const hiddenCount = allSections.filter((section) => settings[section.setting]).length;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-sm font-medium text-gray-900 dark:text-gray-100">Profile Sections</h2>
        ...
        <div className="flex gap-2 mt-4">
          <QuickActionButton
            ...
            onClick={() =>
              handleBulkAction(
                'hideAll',
                sections.repositories.map((s) => s.id)
              )
            }
          />
          <QuickActionButton
