...
import {
  ...
  ViewControls,
  QuickActionButton,
} from '@/popup/components/Supporting';
...
const OrganizationsList: React.FC<{
  ...
  const groupedOrgs = React.useMemo(() => {
    ...
    return organizations.reduce(
      (acc, org) => {
        const key =
          grouping === 'type' ? org.type : hiddenOrgs.includes(org.name) ? 'hidden' : 'visible';
        if (!acc[key]) acc[key] = [];
        acc[key].push(org);
        return acc;
      },
    ...
    <div className="space-y-6">
      {Object.entries(groupedOrgs).map(([group, orgs]) => (
        <div key={group} className="space-y-2">
          ...
          <div className={clsx('gap-2', viewMode === 'grid' ? 'grid grid-cols-2' : 'space-y-2')}>
            <AnimatePresence>
              {orgs.map((org) => (
                <motion.div
                  key={org.name}
...
const OrganizationCard: React.FC<{
  ...
  >
    <div className="flex flex-col items-center text-center space-y-3">
      <img src={org.avatar} alt={org.name} className="w-16 h-16 rounded-xl" />
      <div>
        <h3 className="font-medium text-sm text-gray-900 dark:text-gray-100">{org.name}</h3>
...
const OrganizationListItem: React.FC<{
  ...
  >
    <img src={org.avatar} alt={org.name} className="w-10 h-10 rounded-lg" />
    <div className="flex-1 min-w-0">
      <h3 className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{org.name}</h3>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-baseline gap-2">
          <h2 className="text-sm font-medium text-gray-900 dark:text-gray-100">Organizations</h2>
